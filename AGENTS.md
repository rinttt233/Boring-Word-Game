...
- **`salvager`（残骸回收站）免建造费**：建造成本是 0，只要一个单元 + 一块残骸地。合金是建造与固化的大头（数据库 5 项要 65、主线固化合计 229），缺合金时优先拆残骸；拆空还会**返还一个执行单元**。
...
10. 自此沿主线推进：`db_sulfuric → db_ammonia → db_petrol → db_distill → db_copper → db_refractory → db_rare_earth → db_upkeep → db_unit_bus → db_unit_parallel`，每步都是「recover → fixate → build 设施 → assign」，并保持：记忆 ≥ 40%、有维护站、电站 ≥ 2 座、空闲单元 ≥ 1；中期开始出现「电网已满」就补**电池组/蓄热罐**（储能）。
...
- **自动通关基线（可复现证明「不读源码也能通关」）**：`python -X utf8 tools/blind_test.py --policy ladder --runs 20 --seed-base 100 --max-minutes 300` —— 该阶梯策略只用本向导与 `report` 的字段（不读源码、不用 `dbg`、不注入资源），**20/20 局从零开局通关**（中位约 42 游戏分钟、14 条主线全固化）。加 `--assert-victory` 时未通关会返回非 0（可当回归用）。
