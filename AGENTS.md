...
- **迁移的前置**：所有**主线**知识条目必须已永久固化（`entries` 里非 optional 的 14 条）；未固化会迁移失败。
...
- **煤是开局最紧的东西**：开局只有 **60 煤**，一座电站 0.4/s ≈ **150 秒**就烧完，而找矿/占领/建造都要排队。**省煤技巧**：电量存到 100+ kWh 就 `unassign <电站>` 停烧，等电量降下来再 `assign` 回去 —— 这一手能把「150 秒断煤」变成从容找矿（实测自动通关就是这样活下来的）。
...
- **自动通关基线（可复现证明「不读源码也能通关」）**：`python -X utf8 tools/blind_test.py --policy ladder --runs 20 --seed-base 100 --max-minutes 300` —— 该阶梯策略只用本向导与 `report` 的字段（不读源码、不用 `dbg`、不注入资源），**20/20 局从零开局通关**（平均约 50 游戏分钟、14 条主线全固化）。加 `--assert-victory` 时未通关会返回非 0（可当回归用）。
